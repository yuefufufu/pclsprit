...
//範囲指定
  ...
  pmf.setInputCloud (filtered_cloud2);
  pmf.setMaxWindowSize (1.0);
  pmf.setSlope (0.1f);
  pmf.setInitialDistance (0.1f);
  pmf.setMaxDistance (0.05f);
  pmf.extract (ground->indices);
  extr_pmf.setInputCloud (filtered_cloud2);
  extr_pmf.setIndices (ground);
  extr_pmf.setNegative (true);
  extr_pmf.filter (*filtered_cloud3);
  ...
  seg.setInputCloud (filtered_cloud3);
  seg.segment (*indices, *coefficients);
  seg.setMaxIterations(100);
  seg.setProbability(0.95);
  extr_seg.setInputCloud(filtered_cloud3);
